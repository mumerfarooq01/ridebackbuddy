...
import { User, Mail, KeyRound, CheckCircle, AlertCircle, Monitor, Clock } from "lucide-react";
...
export default function AdminProfilePage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setName(data.name ?? "");
      setEmail(data.email ?? "");
      // Fetch login history once we have the entity ID
    ...
    <div className="max-w-2xl space-y-8">
      ...
      <div className="bg-white rounded-2xl border border-mist shadow-sm p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="font-heading font-semibold text-navy">Account info</h2>
            <p className="text-muted text-xs capitalize">
              {profile?.role} · member since{" "}
              {profile ? new Date(profile.createdAt).toLocaleDateString("en-CA") : ""}
            </p>
          </div>
        </div>
